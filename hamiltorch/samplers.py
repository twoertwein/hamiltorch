...
def fisher(
    ...
    if metric == Metric.JACOBIAN_DIAG:
        ...
        fish = (
            torch.matmul(jac.view(-1, 1), jac.view(1, -1)).diag().diag()
            / normalizing_const
        )
    else:
        hess = util.hessian(
            log_prob.float(), params, create_graph=True, return_inputs=False
        )
    ...
    if jitter is not None:
        ...
        fish += (
            torch.eye(params_n_elements) * torch.rand(params_n_elements) * jitter
        ).to(fish.device)
    if (metric is Metric.HESSIAN) or (metric is Metric.JACOBIAN_DIAG):
        ...
        abs_eigenvalues = (1.0 / torch.tanh(softabs_const * eigenvalues)) * eigenvalues
        fish = torch.matmul(
            eigenvectors, torch.matmul(abs_eigenvalues.diag(), eigenvectors.t())
        )
...
def cholesky_inverse(fish, momentum):
    ...
    y = torch.triangular_solve(
        momentum.view(-1, 1), lower, upper=False, transpose=False, unitriangular=False
    )[0]
    fish_inv_p = torch.triangular_solve(
        y, lower.t(), upper=True, transpose=False, unitriangular=False
    )[0]
    return fish_inv_p
...
def gibbs(
    ...
    if sampler == Sampler.RMHMC:
        dist = torch.distributions.MultivariateNormal(
            ...
            fisher(
                params, log_prob_func, jitter, normalizing_const, softabs_const, metric
            )[0],
        )
    elif mass is None:
...
def rm_hamiltonian(
    ...
    pi_term = ndim * torch.log(2.0 * torch.tensor(pi))

    fish, abs_eigenvalues = fisher(
        params,
        log_prob_func,
        jitter=jitter,
        normalizing_const=normalizing_const,
    ...
    if abs_eigenvalues is not None:
        if util.has_nan_or_inf(fish) or util.has_nan_or_inf(abs_eigenvalues):
            print(
                "Invalid Fisher: {} , abs_eigenvalues: {}, params: {}".format(
                    fish, abs_eigenvalues, params
                )
            )
            raise util.LogProbError()
    else:
    ...
    if util.has_nan_or_inf(hamiltonian):
        print(
            "Invalid hamiltonian, log_prob: {}, params: {}, momentum: {}".format(
                log_prob, params, momentum
            )
        )
        raise util.LogProbError()
    # if NN_flag:
...
def hamiltonian(
    ...
    if sampler == Sampler.HMC:
        ...
        if type(params) is not list:
            ...
            HC = 0.5 * torch.sum((params[0] - params[1]) ** 2) + 0.5 * torch.sum(
                (momentum[0] - momentum[1]) ** 2
            )
            hamiltonian = HA + HB + explicit_binding_const * HC
    elif (
        sampler == Sampler.RMHMC and integrator == Integrator.S3
    ):  # CURRENTLY ASSUMING DIAGONAL
        log_prob = log_prob_func(params)
        ndim = params.nelement()
        pi_term = ndim * torch.log(2.0 * torch.tensor(pi))
        fish, abs_eigenvalues = fisher(
...
def sample(
    ...
    for n in range(num_samples):
        ...
        try:
            ...
            if debug:
                print(
                    "Current Hamiltoninian: {}, Proposed Hamiltoninian: {}".format(
                        ham, new_ham
                    )
                )
            ...
            if rho >= torch.log(torch.rand(1)):
                ...
                params = ret_params[-1]
                if n > burn:
                    leapfrog_params = ret_params[
                        -num_steps_per_sample:
                    ]  ### Might want to remove grad as wastes memory
                    ret_params.extend(
                        leapfrog_params
                    )  # append the current sample to the chain
                if debug:
                    print("REJECT")
...
def define_model_log_prob(
    ...
    for tau in tau_list:
        dist_list.append(
            torch.distributions.Normal(torch.zeros_like(tau_list[0]), tau ** -0.5)
        )
    ...
    def log_prob_func(params):
        ...
        for weights, index, shape, dist in zip(
            model.parameters(), params_flattened_list, params_shape_list, dist_list
        ):
